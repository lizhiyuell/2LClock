...
int get_layer(int n){
    return ceil(log2(1+(TREE_DEGREE_MAX-1)*n)/log2(TREE_DEGREE_MAX));
}
    ...
    case TREE_LAYOUT:
        ...
        for(unsigned int idx=0; idx<server_num; idx++){
            ...
            if(idx==0)
                ...
                father_id = (int)((idx-1)/TREE_DEGREE_MAX);
            
            connection_master[idx][0] = father_id;
            connection_master[idx][1] = -1;

            // fill the slave array
            int start_id = TREE_DEGREE_MAX*idx+1;
            int end_id = std::min((int)server_num, start_id+TREE_DEGREE_MAX)-1;

            int idx_ = -1;
            for(int i=start_id; i<=end_id; i++){
                idx_ = i-start_id;
                connection_slave[idx][idx_] = i;
            }
            connection_slave[idx][idx_+1] = -1;
...
void GTCenter::send_layout_to_master(){
    ...
    for(uint32_t idx=0; idx<server_num; idx++){
        ...
        printf("- Slaves %d:\n", msg->slave_port_num);
        for(int ii=0; ii<msg->slave_port_num; ii++)
            printf("%d\n", msg->slave_port[ii]);
